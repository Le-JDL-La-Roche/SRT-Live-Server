...
int  CHttpClient::open(const char *url, const char *method, int interval) {
  m_begin_tm_ms = get_time_in_milliseconds();
  int ret = SLS_OK;
  strcpy(m_url, url);
  if (NULL == m_url) {
    log(LOG_DFLT, "[%p]CHttpClient::open, failed, m_url is NULL.", this);
    goto FUNC_END;
  }
  if (strlen(m_url) == 0) {
    log(LOG_DFLT, "[%p]CHttpClient::open, failed, m_url='%s'.", this, m_url);
    goto FUNC_END;
  }
  ...
  ret = parse_url();
  if (SLS_OK != ret) {
    log(LOG_DFLT, "[%p]CHttpClient::open, parse_url failed.", this);
    goto FUNC_END;
  }

  char resolved_ip[128];
  if (SLS_OK != get_host_by_name(m_remote_host, resolved_ip)) {
    log(LOG_DFLT, "[%p]CHttpClient::open, failed to resolve, remote_host='%s', remote_port=%d.",
      this, m_remote_host, m_remote_port);
    goto FUNC_END;
  }
  ret = CTCPRole::open(resolved_ip, m_remote_port);
  if (SLS_OK != ret) {
    log(LOG_DFLT, "[%p]CHttpClient::open, failed, remote_host='%s', remote_port=%d.",
      this, m_remote_host, m_remote_port);
    goto FUNC_END;
  }
  //clear response
  ...
  if (SLS_OK != ret) {
    log(LOG_DFLT, "[%p]CHttpClient::open, generate_http_request failed, remote_host='%s', remote_port=%d.",
      this, m_remote_host, m_remote_port);
    goto FUNC_END;
  }
  //send data
...
int  CHttpClient::check_timeout(int64_t cur_tm_ms) {
  ...
  if (cur_tm_ms == 0) {
    cur_tm_ms = get_time_in_milliseconds();
  }
  ...
  log(LOG_DFLT, "[%p]CHttpClient::check_timeout, ok, url='%s', http_method='%s', content_len=%d, m_response_content_length=%d.",
...
int CHttpClient::parse_http_response(std::string &response) {
  if (m_response_info.m_response_header.size() > 0) {
    ...
    if (m_response_info.m_response_content_length == m_response_info.m_response_content.length()) {
      log(LOG_DFLT, "[%p]CHttpClient::parse_http_response, finished, url='%s', http_method='%s', content_len=%d.",
        this, m_url, m_http_method, m_response_info.m_response_content.length());
      m_end_tm_ms = get_time_in_milliseconds();
      if (NULL != m_callback) {
        m_callback(this, HCT_RESPONSE_END, &m_response_info, m_callback_context);
      }
    }
  ...
  if (response_parts.size() == 0) {
    log(LOG_DBUG, "[%p]CHttpClient::parse_http_response, failed, split_string, not found '\r\n\r\n', continue.",
      this);
    return SLS_ERROR;
  }

  split_string(response_parts[0], "\r\n", m_response_info.m_response_header);
  if (m_response_info.m_response_header.size() > 0) {
    //HTTP/1.1 200 OK
    std::vector<std::string> parts;
    split_string(m_response_info.m_response_header[0], " ", parts);
    if (parts.size() == 3) {
      m_response_info.m_response_code = parts[1];//
      log(LOG_DFLT, "[%p]CHttpClient::parse_http_response, m_response_code:'%s', url='%s', http_method='%s'.",
        this, m_response_info.m_response_code.c_str(), m_url, m_http_method);
    }

    //Content-Length
    std::string dst = std::string("Content-Length:");
    std::string content_length = find_string(m_response_info.m_response_header, dst);
    if (content_length.length() > 0) {
      split_string(content_length, ":", parts, 1);
      if (parts.size() == 2) {
        m_response_info.m_response_content_length = atoi(parts[1].c_str());
        log(LOG_DFLT, "[%p]CHttpClient::parse_http_response, m_response_content_length=%d.",
...
int CHttpClient::recv() {
  ...
  if (http_response.length() > 0) {
    ...
    if (SLS_OK == check_finished()) {
      log(LOG_DFLT, "[%p]CHttpClient::recv, finished.",
        this);
    }
  }
  return ret;
...
int CHttpClient::send() {
  ...
  while (true) {
    ...
    if (n < m_out_data_len) {
      ...
      m_out_pos = 0;
      m_out_data_len = 0;
      log(LOG_WARN, "[%p]CHttpClient::send, write failed, m_url='%s', m_out_data_len=%d, but n=%d.",
...
int CHttpClient::handler() {
  ...
  if (ret == -1) {
    log(LOG_DFLT, "[%p]CHttpClient::handler, ret=%d, errno=%d, err='%s'",
...
int CHttpClient::parse_url() {
  ...
  p = strchr(url, ':');
  if (!p) {
    log(LOG_DFLT, "[%p]CHttpClient::parse_url, no ':', url='%s'", this, m_url);
    return SLS_ERROR;
  }
  p[0] = 0x00;
  if (strcmp(url, "http") != 0) {
    log(LOG_DFLT, "[%p]CHttpClient::parse_url, not 'http' prefix, url='%s'.",
      this, m_url);
    return SLS_ERROR;
  }
  p += 3;//skip '://'
  ...
  if (p_tmp) {
    ...
    if (p_tmp) {
      ...
      log(LOG_DFLT, "[%p]CHttpClient::parse_url, no '/' in '%s'.",
        this, m_url);
      m_remote_port = atoi(p);
      p = p_tmp;
    }
...
std:string http_header;
  ...
  if (strcmp(m_http_method, "GET") != 0 && strcmp(m_http_method, "POST") != 0) {
    log(LOG_DFLT, "[%p]CHttpClient::write_http_header, wrong method='%s'.",
      this, m_http_method);
    return SLS_ERROR;
  }
  ...
  if (len != ret) {
    log(LOG_DFLT, "[%p]CHttpClient::write_http_header, failed, m_out_array.put len=%d, ret=%d, url='%s', http_method='%s'.",
...
int CHttpClient::write_string(std::string *str) {
  ...
  if (len > 0) {
    ...
    if (len != ret) {
      log(LOG_DFLT, "[%p]CHttpClient::write_string, failed, m_out_array.put len=%d, ret=%d.",
        this, len, ret);
      return SLS_ERROR;
    }
  }
...
int CHttpClient::generate_http_request() {
  ...
  if (0 >= ret) {
    log(LOG_DFLT, "[%p]CHttpClient::generate_http_request, write_get_header failed, remote_host='%s', remote_port=%d.",
      this, m_remote_host, m_remote_port);
    return ret;
  }
  ...
  if (post_data_len > 0) {
    ...
    log(LOG_DFLT, "[%p]CHttpClient::generate_http_request, write_post_content, ret=%d, remote_host='%s', remote_port=%d.",
      this, ret, m_remote_host, m_remote_port);
  }
  log(LOG_DFLT, "[%p]CHttpClient::generate_http_request, ok, m_url='%s', content len=%d.",
