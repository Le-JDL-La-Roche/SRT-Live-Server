...
int CSLSArray::put(const uint8_t *data, int len) {
  if (NULL == data || len <= 0) {
    log(LOG_ERRO, "Put SLS Array failed.",
      this, data, len);
    return SLS_ERROR;
  }
  ...
  if (len > nRemainder) {
    ...
    log(LOG_ERRO, "Put SLS Array failed, len is bigger than nRemainder");
...
int CSLSArray::get_inline(uint8_t *data, int size) {
  if (NULL == m_arrayData) {
    log(LOG_ERRO, "Get inline SLS Array failed, m_arrayData is NULL.", this);
    return SLS_ERROR;
  }

  if (0 == m_nDataCount) {
    log(LOG_DBUG, "[%p]CSLSArray::get, no new data.", this);
    return SLS_OK;
  }
  log(LOG_DBUG, "[%p]CSLSArray::get, m_nReadPos=%d, m_nWritePos=%d, m_nDataCount=%d, m_nDataSize=%d.",
  ...
  if (m_nReadPos < m_nWritePos) {
    ...
    //log(LOG_DBUG, "[%p]CSLSArray::get, read pos is behind in the write pos, copy_data_len=%d, ready_data_len=%d, size=%d.",
    ...
    copy_data_len = ready_data_len <= size ? ready_data_len : size;
    //log(LOG_DBUG, "[%p]CSLSArray::get, read pos is before of the write pos, copy_data_len=%d, ready_data_len=%d, size=%d.",
    //		this, copy_data_len, ready_data_len, size);
  ...
  log(LOG_DBUG, "[%p]CSLSArray::get, copy_data_lens=%d.",
    this, copy_data_len);
